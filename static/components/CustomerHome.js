...
<div v-else>
  <p>No results found.</p>
    ...
    <div v-for="category in categories" :key="category.c_id" class="category-container">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <h2 class="category-title">{{ category.name }} - {{ category.description }}</h2>
      </div>
      <div class="products-container">
        <div class="product-box" v-for="product in category.products" :key="product.p_id">
  ...
  methods: {
    ...
    async fetchProductsForCategories() {
      for (const category of this.categories) {
        const productsRes = await fetch(`/api/category_product/${category.c_id}`, {
          method: 'GET',
          headers: {
            'Authentication-Token': this.token,
            'Content-Type': 'application/json',
        ...
        if (productsRes.ok) {
          ...
          this.$set(category, 'products', products);
          this.filteredProducts.find(c => c.c_id === category.c_id).filteredProducts = products;
        } else {
          console.error('Error fetching products:', productsRes.status, productsRes.statusText);
        }
      }
    },
